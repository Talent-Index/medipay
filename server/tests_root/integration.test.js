    ...
    describe('RLS Security', () => {
        it('should enforce patient data isolation', async () => {
            ...
            if (patient1Data.data.length > 0 && patient2Data.data.length > 0) {
                const patient1Ids = patient1Data.data.map((r) => r.id);
                const patient2Ids = patient2Data.data.map((r) => r.id);
                const overlap = patient1Ids.filter((id) => patient2Ids.includes(id));
        ...
        it('should enforce doctor access control', async () => {
            ...
            if (doctorData.data.length > 0) {
                doctorData.data.forEach((record) => {
...
describe('Performance Tests', () => {
    ...
    it('should handle concurrent requests', async () => {
        const requests = Array(10).fill(null).map(() => fetch(`${API_BASE_URL}/api/insurance-packages`));
